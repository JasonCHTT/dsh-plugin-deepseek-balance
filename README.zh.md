...
它渲染在原有 token 统计行（`517 / 12.2K · 45.2s …` 那一行）的正下方。鼠标悬停显示明细（赠送 / 充值 / 更新时间），点击任意处立即刷新。
...
│  本次运行已用 ¥0.42 · 余额 ¥10.98           │   ← 本插件（点击刷新）
└──────────────────────────────────────────────┘
```

悬停提示：`赠送 1.23 · 充值 9.75 · 更新于 14:32:05 · 点击刷新`。
